    ...
    <div className='py-3'>
        <ul className="d-flex gap-4  flex-wrap justify-content-center"  >
         {products.map((product ) => (
            <li key={product.id} className="position-relative max-width-475 aspect-square col-12 col-sm-6 col-md-3">
              <Link className="position-relative h-100 w-100" href={`/product${product.id}`}>
                <div className={`${Style.imageContent} d-flex h-100 w-100 aligns-items-center borderColor justify-content-center rounded-lg bg-black`}>
                  <Image className='object-contain' src={product.product_image} layout='fill' />
